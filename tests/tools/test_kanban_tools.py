...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        ...
        "no_task_state_update_reason": "Structured metadata round-trip test only",
        "retry_notes": "none",
...
def test_complete_requires_review_required_when_code_changed(worker_env):
    ...
    retry = kt._handle_complete({
        ...
        "metadata": {
            ...
            "no_commit_reason": "verification-only test case",
            "final_git_status": "dirty with agent-made changes",
            "no_task_state_update_reason": "Validation test only",
            "residual_risk": [],
...
def test_complete_goal_mode_accepts_five_value_done_judge(monkeypatch, tmp_path):
    """Regression for the completion gate unpacking the current judge shape."""
    from pathlib import Path as _Path
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt
